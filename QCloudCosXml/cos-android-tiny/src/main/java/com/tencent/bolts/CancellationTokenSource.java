...
/**
 * Signals to a {@link com.tencent.bolts.CancellationToken} that it should be canceled. To create a
 ...
 *
 * @see com.tencent.bolts.CancellationToken
 * @see CancellationTokenSource#getToken()
 */
public class CancellationTokenSource implements Closeable {

  private final Object lock = new Object();
  private final List<com.tencent.bolts.CancellationTokenRegistration> registrations = new ArrayList<>();
   ...
   */
  public com.tencent.bolts.CancellationToken getToken() {
    synchronized (lock) {
      throwIfClosed();
   ...
   */
  public void cancel() {
    List<com.tencent.bolts.CancellationTokenRegistration> registrations;
    synchronized (lock) {
      throwIfClosed();
  ...
  public void close() {
    synchronized (lock) {
      ...
      List<com.tencent.bolts.CancellationTokenRegistration> registrations = new ArrayList<>(this.registrations);
      for (com.tencent.bolts.CancellationTokenRegistration registration : registrations) {
        registration.close();
      }
  ...
  /* package */ com.tencent.bolts.CancellationTokenRegistration register(Runnable action) {
    com.tencent.bolts.CancellationTokenRegistration ctr;
    synchronized (lock) {
      throwIfClosed();

      ctr = new com.tencent.bolts.CancellationTokenRegistration(this, action);
      if (cancellationRequested) {
        ctr.runAction();
      } else {
  ...
  /* package */ void unregister(com.tencent.bolts.CancellationTokenRegistration registration) {
    synchronized (lock) {
      throwIfClosed();
  ...
  private void notifyListeners(List<com.tencent.bolts.CancellationTokenRegistration> registrations) {
