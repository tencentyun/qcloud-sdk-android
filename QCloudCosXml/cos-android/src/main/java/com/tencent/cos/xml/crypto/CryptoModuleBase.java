...
public abstract class CryptoModuleBase implements CryptoModule {
    ...
    public InitMultipartUploadResult initMultipartUploadSecurely(InitMultipartUploadRequest request)
        ...
        // encrypt object data
        cipherInitMultipartUploadRequest(request);
    ...
    public void uploadPartAsyncSecurely(UploadPartRequest req, CosXmlResultListener resultListener) {
        ...
        try {
            ...
            uploadContext = multipartUploadContexts.get(uploadId);
            if (uploadContext == null) {
                throw CosXmlClientException.internalException(
                        "No client-side information available on upload ID " + uploadId);
            }
            ...
            cos.uploadPartAsync(req, resultListener);
        } catch (CosXmlClientException clientException) {
            resultListener.onFail(req, clientException, null);
        } finally {
            if (uploadContext != null) uploadContext.endPartUpload();
        }
    ...
    protected final InitMultipartUploadRequest cipherInitMultipartUploadRequest(final InitMultipartUploadRequest request) {
    ...
    private CipherLiteInputStream newCOSCipherLiteInputStream(PutObjectRequest req,
        ...
        try {
            ...
            // isCurr = new ResettableInputStream(fileOrig);
            InputStream isCurr = openInputStream(req);
    ...
    public CipherLiteInputStream newCOSCipherLiteInputStream(PutObjectRequest req,
                                                              final CipherLite cipherLite) throws CosXmlClientException {
        try {
